...
16. **Install ordering** — wizard runs `docker image inspect ai-context:latest >/dev/null 2>&1` (the actual image tag emitted by `docker-compose.yml`'s `image:` field; the Compose *service* name is `claude-sandbox` but the *image tag* is `ai-context:latest`); on miss, automatically runs `docker compose -f /opt/.../host/docker-compose.yml --project-directory /opt/.../host build claude-sandbox` before proceeding to steps (e)/(f).

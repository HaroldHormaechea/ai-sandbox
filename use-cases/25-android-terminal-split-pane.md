# Use Case 25: Android terminal — split-pane support (UC04-3c) **[REJECTED — closed 2026-06-08, not implemented]**
...
## Status
**Rejected (2026-06-08).** Closed without dispatch after user review. Split-pane is a sizeable (UC-21-scale) feature addition, and the terminal pain that motivated the 2026-05-30 cleanup has since been addressed by UC-24 (multi-window leak fix), UC-33 (re-zoom on mid-stream subagent spawn), and the keyboard/IME work (UC-23, UC-36). The split-pane feature is not wanted; this UC is retained for historical context only and will not be implemented unless explicitly re-opened.

_Prior tentative note (superseded):_ Surfaced for user review in the 2026-05-30 cleanup run rather than dispatched, because (a) UC-24 should land first, (b) split is a feature addition to a screen the user was still unhappy with, and (c) the multi-layer scope is comparable to UC-21 itself.
